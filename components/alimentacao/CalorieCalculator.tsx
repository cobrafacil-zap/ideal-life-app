...
import { PORTION_TABLE, type PortionItem } from "@/lib/nutrition";
...
/**
 * Calculadora visual de calorias:
 * - busca rápida nos alimentos da tabela;
 * - cada item adicionado tem um contador de porções (1, 2, 3…);
 * - mostra o total agregado em kcal + macros;
 * - botão "Usar no formulário" injeta os totais.
 *
 * Tudo client-side — a tabela tem ~20 itens.
 */
export function CalorieCalculator({ onApply, onApplied }: CalorieCalculatorProps) {
  ...
  const filtered: PortionItem[] = useMemo(() => {
    ...
    if (!q) return PORTION_TABLE;
    return PORTION_TABLE.filter((p) => p.label.toLowerCase().includes(q));
  }, [query]);
    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar alimento (arroz, feijão, frango...)"
      ...
      {/* Lista de alimentos */}
      <ul
        className="max-h-72 space-y-1 overflow-y-auto rounded-2xl border border-line/60 bg-surface"
        role="list"
      ...
      >
        {filtered.map((p) => {
          ...
          return (
            <li
              key={p.label}
              className={cn(
                "flex items-center gap-3 border-b border-line/40 p-3 last:border-b-0 transition-colors",
                n > 0 && "bg-ember-soft/30",
              )}
            >
        ...
        {filtered.length === 0 && (
          <li className="p-6 text-center text-[12px] text-ink-soft">
